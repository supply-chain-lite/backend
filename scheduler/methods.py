...
def log_job_execution(conn, job_id: int, job_name: str, status: str, started_at: str, 
        completed_at: str | None = None, duration: float | None = None, 
        retry_count: int = 0, error_message: str | None = None, 
        result_data: str | None = None) -> int:
    ...
    execution_id = conn.execute( db_queries.insert_job_execution, (job_id, job_name, status, started_at, 
                    completed_at, duration, retry_count, error_message, result_data)).fetchone()
    if not execution_id:
...
def update_job_execution( conn, job_id,  execution_id: int, status: str, completed_at: str, duration: float, 
        error_message: str | None = None, result_data: str | None = None) -> None:
    ...
    conn.execute( db_queries.update_job_execution, (status, completed_at, duration, error_message, 
