from app.config import SECRET_KEY, SMTP_PORT, SMTP_URL, SMTP_USER, SMTP_PWD
from fastapi import HTTPException
import hashlib, os, json, smtplib
...
def register_user(cursor, useremail: str, username: str, password: str):
    ...
    existing = cursor.execute( queries.check_user_email, (useremail,),).fetchone()
    if existing:
    ...
    cursor.execute(queries.create_user, (useremail, 2, username, password_hash, salt, activation_code, 0, 
                                         json.dumps(model_templates)))
    
    subject = "Welcome to Supply Chain Lite"
    body = f"Hello {username},\n\nThank you for registering with Supply Chain Lite! Please activate your account using the following code: {activation_code}\n\nBest regards,\nSCL Team\n"
