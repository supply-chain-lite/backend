...
def forgot_password(cursor, useremail: str, request_url: str):
    row = cursor.execute(queries.get_status_activation_code, (useremail,)).fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    password_reset_code = os.urandom(3).hex()
    cursor.execute(queries.update_password_reset_code, (password_reset_code, useremail))
    cursor.intermediate_commit()


    subject = "Supply Chain Lite Password Reset"
    params = urlencode({"useremail": useremail, "activationcode": password_reset_code})
    reset_link = f"{request_url}/reset-password.html?{params}"
